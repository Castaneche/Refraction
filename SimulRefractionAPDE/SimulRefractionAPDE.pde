...
//Les Positions
PVector p = new PVector(1280/2,720/2); // Le milieu du cercle NOTE:MODIFIER VALEURS 1280 ET 720 EN FONCTION DU TEL
float pxN1 = 1280/2, pyN1 = 0, pxN2 = 1280/2, pyN2 = 720; //Position de la normale  //NOTE:MODIFIER VALEURS 1280 ET 720 EN FONCTION DU TEL
...
void draw()
{
 background(40);
  ...
  demiCercle(width/2,height/2,195,180,360);
...
void checkStatut(){
  ...
  if(info){
    ...
    text("Note: le rayon ne se réfractera pas en fonction de la longueur d'onde (fonction non-implémentée).", width/2, height/5+350);
    textSize(10);
    textAlign(CENTER,DOWN);
    text("Simulateur de la réfraction de la lumière, par cybermissia74 et cynosura, version 4-droid (10/02/2016)", width/2, height-height/9);
    textAlign(CENTER,CENTER);
    textSize(15);
   
    if(bBlock.afficher(width/2-50, height/5+250,100,40, "Quitter")){
      delay(100);
      info = false;
      block = false;
    }
    ...
    {
       textAlign(CENTER,CENTER);
       if(bBlock.afficher(width/2, height-50,150,40,"Afficher de l'aide") == true && info == false){
         delay(100);
         info = true;
         block = true;
       }
       if(bBlock.afficher(width/2-150, height-50,150,40,"Mettre en pause") == true && block == false){
         block = true;
         delay(100);
       }
       else if(block == true && bBlock.afficher(width/2-150, height-50,150,40,"Quitter la pause") == true){
         block = false;
         delay(100);
       }
    }
